...
def _source_heads(y_train, n_passes=1, **model_kwargs):
    ...
    with torch.no_grad():
        for _ in range(n_passes):
            out = model.forward(x, None, None, None)
...
def test_time_llm_short_term_is_forward_alias_of_long_term():
    ...
    np.testing.assert_allclose(short_source, long_source, rtol=1e-5, atol=1e-4)

    long_wrapper = _wrapper_head(y_train, fh, **long_kwargs)
    short_wrapper = _wrapper_head(y_train, fh, **short_kwargs)
    np.testing.assert_allclose(short_wrapper, long_wrapper, rtol=1e-5, atol=1e-4)
    np.testing.assert_allclose(
        short_wrapper[:3],
        short_source[:3],
        rtol=1e-5,
...
def test_time_llm_gpt2_predict_matches_vendored_model_forward():
    ...
    if not np.allclose(source_a, source_b, rtol=1e-5, atol=1e-4):
        pytest.skip(
